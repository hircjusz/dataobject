...
//            pEnvironment.RegisterFunction(typeof(CountFunction));
    ...
    public class SumFunction : RPNAggregateFunction
    {
        ...
        {
            base.Aggregate(Owner);
            Value += Convert.ToDouble(Owner.RPNList[Owner.Position].GetValue(Owner));
        }
    }
    ...
    public class MaxFunction : RPNAggregateFunction
    {
        public override void Aggregate(RPNExpr Owner)
        {
            bool b = Count == 0;
            base.Aggregate(Owner);
            double number = Convert.ToDouble(Owner.RPNList[Owner.Position].GetValue(Owner));
            if (b || number > Value)
                Value = number;
        }
    }
